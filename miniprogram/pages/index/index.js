...
Page({
  ...
  onShow() {
    if (this.data.isLoggedIn) {
      this.setData({ _lastLoadTime: null });
      this.refreshData();
    }
  },
  ...
  generateWeekData() {
    if (!this.data.firstWeekStartDate) return;
  ...
  async login() {
    ...
    try {
      ...
      this.setData({ isLoggedIn: true, userInfo, isLoading: false, syncStatus: '同步完成' });
      this.refreshData();
      wx.hideLoading();
  ...
  async loadMainTimetable() {
    ...
    try {
      ...
      const settings = localApi.getSettings(mainTimetable._id);
      if (settings?.timeSlots?.length > 0) {
        this.setData({ timeSlots: settings.timeSlots });
      }

      const courseCount = this.calculateCourseCount(mainTimetable?.courses);
      ...
      const weekData = this.data.weekData.map(weekItem => {
        ...
        const courses = mainTimetable?.courses || [];
      ...
      const hasCourse = mainTimetable?.courses?.some(day => day.some(course => course != null));

      this.setData({
        mainTimetableId: mainTimetable?._id,
        mainTimetableName: mainTimetable?.name || '我的课表',
        weekData, isEmpty: !hasCourse, courseCount, syncStatus: ''
      });
    } catch (err) {
  ...
  loadFromLocal() {
    ...
    if (mainTimetable) {
      ...
      this.setData({
        mainTimetableId: mainTimetable._id || mainTimetable.id,
        mainTimetableName: mainTimetable.name || '我的课表',
        weekData,
        isEmpty: !mainTimetable.courses?.flat().some(c => c),
        courseCount
      });
    }
  },
